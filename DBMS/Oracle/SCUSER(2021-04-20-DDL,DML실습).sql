...
-- 데이터 Type
-- 문자열 데이터
--      CHAR : 저장되는 데이터의 (규칙으로)길이가 정해진경우, 데이터 길이가 모두 같은 경우
--          "고정길이 문자열"
...
--          "가변길이 문자열"
--      nVARCHAR  : 한글과 같은 UNICODE 문자열을 저장할떄
--          영문알파벳과 저장하는 방식이 달라서
--          문제를 일으킬수 있다
...
CREATE TABLE tbl_score (
    sc_num CHAR(5) , -- 문자열 5자리의 데이터만 저장, Strin num
...
--      생성된 Table에 데이터 추가, 최회, 수정, 삭제를 하는 명령어
-- DBMS 대표적인 CRUD 업무
