...
PRIMARY KEY(bk_isbn);  -- bk_isbn칼럼을 PK로 설정하겠다
...
저장 항목(칼럼)을 보면 저자가 2명 이상인 데이터가 있고 
또한 역자도 2명 이상인 경우가 있다

데이터를 저장할 칼럼을 크게 설정하여 입력(import)하는데는
문제가 없는데
저자나 역자를 기준으로 데이터를 여러가지 조건을 부여하여
조회를 하려고 하면 문제가 발생할수 있다
특히 저자이름으로 GOUPPING을 하여 데이터를 조회해 보려고 하면
상당히 어려움을 겪을 수 있다
...
RENAME COLUMN bk_author TO bk_author1; -- bk_author 칼럼을 bk_autor1
...
제 2정규화를 통하여 테이블 설계를 다시 해야 하겠다
1. 정규화를 수행할 칼럼이 무엇인가 파악(인식한다)
    저자 데이터를 저장할 칼럼
    복수의 데이터가 필요한 경우
    
2. 도서정보와 관련된 저자 데이터를 저장할 Table새로 생성한다
...
-- 제1 정규화가 완료된 도서정보로 부터 저자리스트를 생성
...
-- 테이블의 ba_seq칼럼에는 이미 등록된 값이 아닌
-- 새로운 숫자를 사용하여 데이터를 추가해야 한다
-- 데이터를 추가할때마다 새로운 값이 무엇인지 알아야 하는 
-- 매우 불편한 상황이 만들어지고 말았다.
INSERT INTO tbl_book_author( bs_seq, ba_isbn, ba_author)
